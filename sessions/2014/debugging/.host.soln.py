...
######## Runtime Error 2: Range off-by-one, should ask for 10 objects
...
######## Syntax Error 1: Need comma between each item
...
def CheckTables():
	avail = []
	for place in tables:
		#########Logic Error 1: Should check that status available
...
######## Runtime Error 1: Dictionary should use curly braces {}
wait = []
...
######## Syntax Error 2: While statement should end with colon, not semicolon
while 1;
	# Ask the user how many are in their party
	size = input("\nAkwaba! How many people are in your party? ")
	
	######### Validation Error 1: Don't handle the exception of a user entering a non-number
	size = int(size)
	
	# Check if this party will fit with any table availability
	# Discern availability of tables inside a function
	######## Runtime Error 3: Function call must include parentheses
	...
	if avail_tables:
		...
		#########Logic Error 2: Should have checked size < 0; program cannot exit cleanly
		...
		#########Validation Error 2: Dangerous to allow eval() to execute arbitrary code
		try: eval(w)
